...
class JQTabla{
  ...
  if(empty($datos->camposelect)){
    ...
    $sql = "SELECT ".implode(",",$datos->campos)." FROM ".$datos->tabla.$condicionConteo . $econdicion." ORDER BY $sidx $sord " ." LIMIT ".$start.", ".$final;
	//$sql = "SELECT  ".implode(",",$datos->campos )." FROM (SELECT  ".implode(",",$campos).", ROW_NUMBER() OVER (ORDER BY $sidx $sord) R FROM ".$datos->tabla.$condicionConteo.$econdicion.") as R WHERE ".$condicion." R BETWEEN ".$start." AND ".$final." ORDER BY $sidx $sord";
    $q = $CI->db->query($sql);
