...
class Lib_usuarios {
    ...
    private function getUsuarioPerfil($US_CODIGO = null){
     ...
     public function getAccesoSeccion($UXR_UCOD = null,$USD_ALIAS = null){
        $CI =& get_instance();
		$sql="SELECT USP_SECUENCIAL 
				FROM VW_USUARIOXDIRXPER 
				WHERE APLICACION=(SELECT APP_SECUENCIAL FROM APLICACION WHERE APP_CODIGO='APP_GESTOR') 
				AND USUARIO=(SELECT US_SECUENCIAL FROM USUARIO WHERE UPPER(US_CODIGO)=UPPER('{$UXR_UCOD}'))
				AND ALIAS='{$USD_ALIAS}'";
    ...
    public function getElementoAcceso($US_CODIGO = null,$USD_SECUENCIAL = null){
        ...
        $value = $CI->db->select("USP_ELEMENTO")
                 ->where("USP_SEC_ROLES = {$UXR_RSEC} AND USP_SEC_DIRECTIVAS = {$USD_SECUENCIAL} AND USP_ESTADO = 0")
                 ->get("PERMISOS")
                 ->row_array();
