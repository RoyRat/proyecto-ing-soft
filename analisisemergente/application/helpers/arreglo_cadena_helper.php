<?php  if (!defined('BASEPATH')) exit('No direct script access allowed');
	...
	function prepararNombreArchivo($str = null){
        return is_string($str)?(regCaracteresEspecialers(trim(fullUpper($str)))):$str;
	}
	
    function prepCampoMostrar($str){
        return is_string($str)?(trim($str)):trim(($str));
	}
	...
	*/
	...
	function encriptar($string){
		...
		$qEncoded=base64_encode(mcrypt_encrypt(MCRYPT_RIJNDAEL_256,md5($key),$result,MCRYPT_MODE_CBC,md5(md5($key))));
		return $qEncoded;
	}
